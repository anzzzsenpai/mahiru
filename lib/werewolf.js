...
async function pagi(mahiru, x, data) {
    ...
    return await mahiru.sendMessage(x.room, {
        text: pagii(x),
        contextInfo: {
            externalAdReply: {
...
async function voting(mahiru, x, data) {
    let row = [];
    let ment = [];
...
async function malam(mahiru, x, data) {
...
async function skill(mahiru, x, data) {
    skillOn(x.room, data)
        ...
        for (let i = 0; i < x.player.length; i++) {
            if (x.player[i].role === "werewolf") {
                if (x.player[i].isdead != true) {
                    ...
                    await mahiru.sendMessage(x.player[i].id, {
                        text: textnya,
                        mentions: membernya,
                    });
                }
...
async function win(x, t, mahiru, data) {
    ...
    if (getWinner(x.room, data).status === false || x.iswin === false) {
        ...
        return await mahiru.sendMessage(sesinya, {
            text: textnya,
            contextInfo: {
                externalAdReply: {
                    title: "W E R E W O L F",
...
// playing
async function run(mahiru, id, data) {
    while (getWinner(id, data).status === null) {
        ...
        if (getWinner(id, data).status != null) {
            ...
            await pagi(mahiru, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, hydro, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await voting(mahiru, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, hydro, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await malam(mahiru, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, hydro, data);
            break;
        } else {
            await skill(mahiru, sesi(id, data), data);
        }
...
async function run_vote(mahiru, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, hydro, data);
            break;
...
async function run_malam(mahiru, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, hydro, data);
            break;
...
async function run_pagi(mahiru, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, hydro, data);
            break;
