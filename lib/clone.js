...
const jadibot = async (mahiru, m, from) => {
  if (Object.keys(client).includes(from)) {
    return mahiru.sendMessage(from, {
    ...
    async function connectToWhatsApp() {
      ...
      if (usePairingCode && !client[from].user && !client[from].authState.creds.registered) {
        setTimeout(async () => {
          ...
          await mahiru.sendMessage(from, {text: txt }, {quoted: m})
        }, 2000)
      }
      ...
      client[from].ev.on('messages.upsert', async chatUpdate => {
        try {
          ...
          require('../mahiru')(client[from], m, chatUpdate, store)
        } catch (err) {
          console.log(err)
        }
      })
...
async function stopjadibot(mahiru, m, from) {
  if (!Object.keys(client).includes(from)) {
    return mahiru.sendMessage(from, {
...
async function listjadibot(mahiru, m) {
